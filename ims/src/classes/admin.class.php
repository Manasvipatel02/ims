...
class Admin extends Config
{
    ...
    public function create_student($student_name, $student_phone_number, $email, $password, $role,$class)
    {
        ...
        if (isset($role) && isset($student_name) && isset($student_phone_number) && isset($email) && isset($password) && strlen($password) > 3) {
            ...
            $insert = $this->db->query("INSERT INTO `user`(`name`, `email`, `role`, `password`, `phone_number`,`class`) VALUES (?,?,?,?,?,?)", $student_name,$email,$role,$password, $student_phone_number,$class);
    ...
    // View all student
    public function view_student($student_id, $single = false,$class=0)
    {
        ...
        if ($single == true && $student_id != 0) {
            ...
            // print_r($class);die;
            $insert = $this->db->query("SELECT * FROM user WHERE class NOT LIKE ? and role like ?", $class, 'student');
            // print_r($insert);rtr

            // $insert = $this->db->query("SELECT * FROM user WHERE  role like ?", 'student');
        } else{
    ...
    public function create_teacher($teacher_name, $teacher_phone_number, $email, $password)
    {
        ...
        if (isset($teacher_name) && isset($teacher_phone_number) && isset($email) && isset($password) && strlen($password) > 5 ) {
            ...
            $insert = $this->db->query("INSERT INTO `user`(`name`,  `email`, `role`,`phone_number`, `password`,`class`) VALUES (?,?,?,?,?,0)", $teacher_name,$email, 'teacher',$teacher_phone_number, $password);
    ...
    public function update_teacher($teacher_id, $teacher_name, $teacher_phone_number, $email, $password)
    {
        ...
        if (isset($teacher_id) && isset($teacher_name) && isset($teacher_phone_number) && isset($email) && isset($password)) {
            ...
            $insert = $this->db->query("UPDATE `user` SET `name`=?, `email`=?,`role`=?, `password`=?, `phone_number`=? WHERE `uid`=?", $teacher_name, $email, 'teacher', $password,$teacher_phone_number, $teacher_id);
